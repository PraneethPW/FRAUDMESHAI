...
async def _summarize(payload: AssistantRequest, user: CurrentUser, db: DB) -> dict:
    ...
    subject_type: str
    subject_id: str
    if payload.alert_id and payload.case_id:
        raise HTTPException(422, "Choose exactly one alert or case")
    if payload.alert_id:
        alert = (
            await db.execute(select(Alert).where(Alert.id == payload.alert_id, Alert.workspace_id == user.workspace_id))
        ).scalar_one_or_none()
        if not alert:
        ...
        evidence = {
            ...
            "structured_evidence": alert.evidence,
        }
        subject_type, subject_id = "ALERT", alert.id
    elif payload.case_id:
        case = (
            await db.execute(select(Case).where(Case.id == payload.case_id, Case.workspace_id == user.workspace_id))
        ).scalar_one_or_none()
        if not case:
            raise HTTPException(status_code=404, detail="Case not found")
        from app.api.cases import get_case

        case_detail = await get_case(case.id, user, db)
        evidence = {
            ...
            "structured_evidence": case.evidence,
        }
    ...
    if not provider:
        return {
            "available": False,
            "message": "AI Investigation Assistant unavailable. Core fraud detection remains operational.",
            "evidence": evidence,
        }
    try:
        ...
        return {
            "available": False,
            "message": "AI provider is temporarily unavailable. Review the structured evidence below; core detection remains operational.",
            "evidence": evidence,
        }
    summary = AISummary(
