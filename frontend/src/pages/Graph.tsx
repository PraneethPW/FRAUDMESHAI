...
export default function GraphExplorer() {
  ...
  const { data, isLoading, error } = useQuery<GraphResponse>({ queryKey: ['graph',type,minRisk,hours], queryFn: () => api.get(`/graph?${type==='ALL'?'':`node_type=${type}&`}min_risk=${minRisk}&limit=300&hours=${hours}`).then((response)=>response.data) })
  useEffect(() => {
    ...
    graph.current = cytoscape({ container: container.current, elements: [...data.nodes,...data.edges], minZoom:.25, maxZoom:3, wheelSensitivity:.22, style: [
      ...
      ...Object.entries(nodeColors).map(([kind,color])=>({selector:`node[type = "${kind}"]`,style:{'background-color':color}})),
  ...
  const expandNode = async (id:string) => {
    try { const {data:extra}=await api.get<GraphResponse>(`/graph/node/${id}`);const cy=graph.current;if(!cy)return;cy.add([...extra.nodes,...extra.edges].filter((e)=>cy.getElementById(e.data.id).empty()));cy.layout({name:'cose',animate:false}).run();cy.fit(cy.getElementById(id).closedNeighborhood(),80) } catch(e){toast.error(errorMessage(e))}
  }
  useEffect(()=>{if(data&&params.get('node'))void expandNode(params.get('node')!)},[data,params])
...
export function FraudRings() {
  ...
  const { data=[],isLoading,error,refetch }=useQuery<Ring[]>({queryKey:['rings'],queryFn:()=>api.get('/fraud-rings').then((response)=>response.data),refetchInterval:30000})
  const createCase=useMutation({mutationFn:(ring:Ring)=>api.post<InvestigationCase>('/cases',{title:`Investigation · ${ring.name}`,severity:ring.risk_level,evidence:{ring_id:ring.id,...ring.properties}}),onSuccess:({data})=>{toast.success('Investigation case created from ring');void refetch();navigate(`/app/cases/${data.id}`)},onError:(err)=>toast.error(errorMessage(err))})
  if(isLoading)return <GraphLoader label="Detecting connected suspicious subgraphs"/>
  if(error)return <ErrorState message={errorMessage(error)}/>
  return <><PageIntro eyebrow="Coordinated threat discovery" title="Fraud ring detector" text="Review candidate rings inferred from shared devices within a 24-hour event window." actions={<div className="detector-state"><Sparkles/><span><b>Shared-device detection active</b><small>Time-windowed, currency-specific evidence</small></span></div>}/>{data.length?<div className="ring-grid">{data.map((ring,index)=><Panel key={ring.id} className="ring-card"><div className="ring-head"><div><span>RING / {String(index+1).padStart(3,'0')}</span><h2>{ring.name}</h2></div><ScoreRing value={ring.risk_score} size={72}/></div><div className="ring-topology"><svg viewBox="0 0 420 170"><path d="M55 85L150 40L250 90L360 35M55 85L155 140L250 90L355 138M150 40L155 140"/></svg>{ring.members.slice(0,6).map((member,nodeIndex)=><i key={member.id} className={`ring-node ring-node-${nodeIndex}`} title={member.id}/>)}</div><div className="ring-reason"><Radar/><p>{ring.reason}</p></div><div className="ring-facts"><div><span>Accounts</span><b>{ring.members.filter((member)=>member.type==='ACCOUNT').length}</b></div><div><span>Shared device</span><b>{String(ring.properties.shared_device??'—')}</b></div><div><span>Suspicious amount</span><b>{new Intl.NumberFormat('en-US',{style:'currency',currency:String(ring.properties.currency??'USD'),maximumFractionDigits:0}).format(ring.estimated_amount)}</b></div></div><div className="ring-actions"><RiskBadge level={ring.risk_level}/><button onClick={()=>createCase.mutate(ring)} disabled={createCase.isPending}><Boxes/> Create case</button></div></Panel>)}</div>:<EmptyState title="No fraud rings detected yet" text="Start Simulation Mode. FraudMesh will surface a ring when multiple accounts converge on suspicious infrastructure."/>}</>
}
